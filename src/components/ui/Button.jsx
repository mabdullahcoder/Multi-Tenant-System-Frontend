function Button({ children, variant = 'primary', size = 'md', disabled = false, loading = false, icon: Icon, iconPosition = 'left', className = '', ...props }) {
    const base = 'inline-flex items-center justify-center gap-2 font-semibold rounded-lg transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95';
    ...
    const variants = {
        ...
        ghost: 'focus:ring-gray-400 active:scale-95',
    };
    ...
    // ghost uses CSS vars for dark mode
    const ghostStyle = variant === 'ghost'
        ? { color: 'var(--text-secondary)' }
        : {};

    // outline uses CSS vars for dark mode
    const outlineStyle = variant === 'outline'
        ? { borderColor: 'var(--primary)', color: 'var(--primary)' }
        : {};

    const combinedStyle = { ...secondaryStyle, ...ghostStyle, ...outlineStyle };
    ...
    return (
        <button
            ...
            onMouseEnter={variant === 'ghost' ? (e) => { e.currentTarget.style.backgroundColor = 'var(--bg-hover)'; e.currentTarget.style.color = 'var(--text-primary)'; } : undefined}
            onMouseLeave={variant === 'ghost' ? (e) => { e.currentTarget.style.backgroundColor = ''; e.currentTarget.style.color = 'var(--text-secondary)'; } : undefined}
            {...props}
        >
            {loading && (
